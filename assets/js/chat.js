/* ════════════════════════════════════════════════════════
   ...
   Based on v3.3 — preserves all original patterns exactly.
   Adds: game detection overlay, rich presence, bug fixes.
════════════════════════════════════════════════════════ */
...
try{PROF=(()=>{
  ...
  return[p('fuck'),p('fuk'),p('fck'),/ph[uv][ck]+/gi,p('shit'),/a+[s$][s$]+/gi,p('bitch'),p('cunt'),/d[i1!][ck]+/gi,p('bastard'),/wh[o0]r[e3]?/gi,/sl[uv][t7]/gi,/n[i1!][g9][g9][e3]r/gi,/n[i1!][g9][g9][a4]/gi,/f[a4][g9][g9][o0][t7]/gi];
})();}catch(e){}
...
/* FIX: robust UTC timestamp parsing */
function parseTS(ts){
...
function formatDate(ts){const d=parseTS(ts),t=new Date(),y=new Date();y.setDate(t.getDate()-1);if(d.toDateString()===t.toDateString())return'Today';if(d.toDateString()===y.toDateString())return'Yesterday';return d.toLocaleDateString([],{month:'long',day:'numeric',year:'numeric'});}
...
/* ── Ephemeral message renderer ───────────────────── */
function showEphemeral(text,icon='🤖',label='Only you can see this'){
  const win=document.getElementById('dc-messages');
  const el=document.createElement('div');el.className='dc-ephemeral';
  el.innerHTML=`<div class="dc-ephemeral-icon">${icon}</div><div class="dc-ephemeral-body"><div class="dc-ephemeral-label">${esc(label)}</div><div class="dc-ephemeral-text">${renderText(text)}</div></div><button class="dc-ephemeral-dismiss" title="Dismiss">✕</button>`;
...
/* FIX: always re-fetch mute status from DB, never trust stale cache */
async function isMutedFresh(userId){
  try{
    ...
    return parseTS(data.muted_until)>new Date();
  }catch{return false;}
}
function isMuted(p){return p?.muted_until&&parseTS(p.muted_until)>new Date();}
function muteExpiryText(p){if(!p?.muted_until)return'';const ms=parseTS(p.muted_until)-new Date();if(ms<=0)return'';const m=Math.ceil(ms/60000);return m<60?`${m}m`:Math.ceil(m/60)+'h';}
...
/* ══════════════════════════════════════════════════════
   GAME DETECTION — reads window title from BroadcastChannel
   (works cross-tab: game tab posts its title, chat tab listens)
   Also polls the current tab's own title every 5s as fallback.
══════════════════════════════════════════════════════ */
const KNOWN_GAMES=[
  ...
  {name:'Stardew Valley',patterns:['stardew valley']},
  {name:'Minecraft Bedrock',patterns:['minecraft for windows']},
  {name:'Steam',         patterns:['steam']},
  {name:'360 Games',     patterns:['minesweeper — 360','360fish','chess — 360','360 snake']},
];
const GAME_EMOJIS={
  'Minecraft':'⛏️','Minecraft Bedrock':'⛏️','Roblox':'🎮','Fortnite':'💥','Valorant':'🔫',
  'Among Us':'🔪','League of Legends':'🏆','CS2':'💣','Apex Legends':'👑',
  'Genshin Impact':'✨','Rocket League':'🚀','Overwatch':'⚔️','Call of Duty':'🪖',
  'GTA V':'🚗','Elden Ring':'🗡️','Terraria':'🌳','Stardew Valley':'🌾',
  'Steam':'🎮','360 Games':'🎮',
};

let currentGame=null;
let _gameBC=null; // BroadcastChannel

function matchGameFromTitle(title){
  const t=(title||'').toLowerCase();
  for(const g of KNOWN_GAMES){if(g.patterns.some(p=>t.includes(p)))return g.name;}
  return null;
}
...
function setCurrentGame(name){
  ...
  updateGameOverlay();
  broadcastPresence();
  // Show toast when game detected/cleared
  if(name)showToast((GAME_EMOJIS[name]||'🎮')+' Now playing: '+name,3500);
}

function updateGameOverlay(){
  let el=document.getElementById('game-overlay');
  if(!el){
    el=document.createElement('div');el.id='game-overlay';el.className='game-overlay';
    el.style.cssText='position:fixed;bottom:80px;right:16px;z-index:4000;animation:goSlideIn .3s cubic-bezier(.34,1.56,.64,1);';
    document.body.appendChild(el);
  }
  if(!currentGame){el.style.display='none';return;}
  el.style.display='';
  const emoji=GAME_EMOJIS[currentGame]||'🎮';
  el.innerHTML=`<div style="display:flex;align-items:center;gap:10px;background:rgba(15,17,23,.95);border:1px solid rgba(99,102,241,.4);border-radius:14px;padding:10px 14px;box-shadow:0 8px 32px rgba(0,0,0,.4);min-width:200px;max-width:280px;backdrop-filter:blur(16px);"><span style="font-size:26px;flex-shrink:0;">${emoji}</span><div style="flex:1;min-width:0;"><div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:#818cf8;margin-bottom:2px;">Now Playing</div><div style="font-size:14px;font-weight:700;color:#f8fafc;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${esc(currentGame)}</div></div><button onclick="this.closest('#game-overlay').style.display='none'" style="background:none;border:none;cursor:pointer;color:rgba(255,255,255,.4);font-size:16px;flex-shrink:0;padding:2px 4px;line-height:1;" title="Dismiss">✕</button></div>`;
}

/* BroadcastChannel: other tabs (game tabs) can post their window title */
function startGameDetection(){
  // Poll own title
  setCurrentGame(matchGameFromTitle(document.title));
  setInterval(()=>setCurrentGame(matchGameFromTitle(document.title)),5000);

  // Cross-tab: listen for game title broadcasts from other tabs
  try{
    _gameBC=new BroadcastChannel('360_game_detect');
    _gameBC.onmessage=e=>{
      if(e.data?.type==='title'){
        const detected=matchGameFromTitle(e.data.title);
        // Only switch if a game is actually detected (don't clear on non-game titles)
        if(detected)setCurrentGame(detected);
        else if(currentGame&&!matchGameFromTitle(document.title))setCurrentGame(null);
      }
    };
  }catch(e){}

  // Document visibility: re-check when tab becomes visible
  document.addEventListener('visibilitychange',()=>{
    if(!document.hidden)setCurrentGame(matchGameFromTitle(document.title));
  });
}

/* Any other 360 page can broadcast its title by including this snippet:
   new BroadcastChannel('360_game_detect').postMessage({type:'title',title:document.title})
   The games page already does this via wide.js. */
...
function startPresence(){
  ...
  chan.on('presence',{event:'sync'},()=>{
    ...
    if(s==='SUBSCRIBED')await broadcastPresence(chan);
  });
  window._presenceChan=chan;
}

async function broadcastPresence(chan){
  const c=chan||window._presenceChan;if(!c||!currentUserId)return;
  await c.track({
    uid:currentUserId,
...
async function buildRail(){
  ...
  gen.title='General';gen.dataset.serverId='';gen.innerHTML='🌐';
...
async function buildSidebar(server){
  ...
  if(!server){
    ...
    addSidebarBtn(body,'🔍 Browse Servers',()=>browseSidebar(body));return;
  }
...
function addCategoryHeader(body,label,onAdd){
  ...
  c.addEventListener('click',e=>{
    ...
    items.forEach(el=>el.style.display=hidden?'':'none');c.classList.toggle('collapsed',!hidden);
  });
  body.appendChild(c);
}
function makeChanItem(ch,isActive,onClick){
  const item=document.createElement('div');item.className='dc-ch-item'+(isActive?' active':'');item.dataset.chId=ch.id;
...
document.getElementById('sb-server-menu')?.addEventListener('click',async(e)=>{
  ...
  if(canManage){
    ...
    items.push({sep:true});
    if(isOwner)items.push({label:'🗑 Delete Server',danger:true,fn:()=>deleteServer(server)});
  }
  if(!isOwner&&joinedServerIds.has(server.id))items.push({label:'🚪 Leave Server',danger:true,fn:()=>leaveServer(server)});
  items.forEach(item=>{
  ...
  /* FIX: clamp so menu doesn't clip off right edge */
  menu.style.left=Math.max(8,Math.min(rect.right,window.innerWidth-210))+'px';
...
function switchRoom(room){
  ...
  if(room.type==='channel'){
    chan.on('postgres_changes',{event:'INSERT',schema:'public',table:'messages',filter:`channel_id=eq.${room.id}`},p=>{
        if(p.new.thread_id!=null)return;
        onIncoming(p.new);
      })
      .on('postgres_changes',{event:'UPDATE',schema:'public',table:'messages',filter:`channel_id=eq.${room.id}`},p=>{
        const el=msgElMap.get(String(p.new.id));
        if(el){updateThreadPill(el,p.new);patchMsgEl(el,p.new);}
      })
      .on('postgres_changes',{event:'DELETE',schema:'public',table:'messages',filter:`channel_id=eq.${room.id}`},p=>{
        msgElMap.get(String(p.old.id))?.remove();msgElMap.delete(String(p.old.id));
      });
    ...
    chan.on('postgres_changes',{event:'INSERT',schema:'public',table:'messages',filter:`server_id=eq.${room.id}`},p=>{if(p.new.thread_id!=null)return;onIncoming(p.new);});
  }else{
    chan.on('postgres_changes',{event:'INSERT',schema:'public',table:'messages'},p=>{if(!p.new.channel_id&&!p.new.dm_id&&!p.new.server_id&&p.new.thread_id==null)onIncoming(p.new);});
  }
  ...
  typingChannel=sb.channel('typing-'+room.type+'-'+room.id)
    .on('broadcast',{event:'typing'},p=>{
      ...
      /* FIX: capture uid in closure */
      const _uid=uid;
      typingTimeouts[_uid]=setTimeout(()=>{delete typingUsers[_uid];renderTyping();},2500);
    })
    .subscribe();

  window.ChatNotif?.onRoomSwitch(room);
  const key=getRoomKey(room);unreadCounts[key]=0;
...
async function fetchMessages(beforeDate){
  ...
  else if(activeRoom.type==='channel')q=sb.from('messages').select('*').eq('channel_id',activeRoom.id).is('deleted_at',null).is('thread_id',null);
  else if(activeRoom.type==='server')q=sb.from('messages').select('*').eq('server_id',activeRoom.id).is('deleted_at',null).is('thread_id',null);
  else q=sb.from('messages').select('*').is('channel_id',null).is('dm_id',null).is('server_id',null).is('deleted_at',null).is('thread_id',null);
...
function buildMsgEl(msg){
  ...
  if(msg.thread_id!=null)return null;
  ...
  if(!sameAuthor){
    ...
    const badgeColors={admin:'#ef4444',mod:'#f59e0b',vip:'#8b5cf6'};
    if(msg.role&&badgeColors[msg.role]){const b=document.createElement('span');b.className='dc-msg-role-badge';b.style.background=badgeColors[msg.role];b.textContent=msg.role;hdr.appendChild(b);}
    ...
    /* Game badge — show if this user is currently playing something */
    const userPresence=Object.values(presenceState).flat().find(u=>u.uid===msg.user_id);
    if(userPresence?.game){const gb=document.createElement('span');gb.title='Playing '+userPresence.game;gb.textContent=GAME_EMOJIS[userPresence.game]||'🎮';gb.style.cssText='font-size:13px;cursor:default;';hdr.appendChild(gb);}
  ...
  pill.style.display=(tc>0||msg.is_thread_root)?'':'none';
  pill.textContent=tc>0?`🧵 ${tc} repl${tc===1?'y':'ies'}`:'🧵 Start Thread';
  pill.addEventListener('click',()=>openThread(msg));
  pill.dataset.threadPill='1';
  ...
  [{i:'↩',t:'Reply',fn:()=>setReply(msg)},{i:'😊',t:'React',fn:ev=>openReactionPicker(msg.id,ev)},{i:'🧵',t:'Thread',fn:()=>openThread(msg)},{i:'📌',t:'Pin',fn:()=>pinMsg(msg)}].forEach(a=>{
...
function updateThreadPill(el,msg){
  ...
  if(count>0||msg.is_thread_root){pill.textContent=count>0?`🧵 ${count} repl${count===1?'y':'ies'}`:'🧵 Start Thread';pill.style.display='';}
}

function renderText(raw){
  if(!raw)return'';
  let t=esc(filterProfanity(applyShortcodes(raw)));
  ...
  t=t.replace(/@(\w+)/g,(m,name)=>{
    ...
    const style=isMe?'color:#fbbf24;background:rgba(251,191,36,.18);border-radius:3px;padding:0 3px;font-weight:700;':'color:var(--a);background:rgba(59,130,246,.12);border-radius:3px;padding:0 3px;font-weight:600;';
...
/* ══════════════════════════════════════════════════════
   REACTIONS
══════════════════════════════════════════════════════ */
async function loadReactionsSingle(msgId){const{data}=await sb.from('reactions').select('emoji,user_id').eq('message_id',msgId);if(data)renderReactions(msgId,data);}
...
document.getElementById('ctx-copy').onclick=()=>{const t=msgElMap.get(String(ctxTargetMsg?.id))?.querySelector('.dc-msg-text')?.textContent||'';navigator.clipboard.writeText(t).then(()=>showToast('📋 Copied!'));};
...
async function loadThreadMsgs(rootId){
  ...
  if(!data?.length){list.innerHTML=`<div style="padding:20px;text-align:center;font-size:13px;color:var(--dc-muted);">No replies yet. Start the conversation!</div>`;return;}
...
function appendThreadMsg(m,skipScroll=false){
  ...
  el.innerHTML=`<div style="display:flex;align-items:baseline;gap:8px;margin-bottom:3px;"><span style="font-size:13px;font-weight:700;color:var(--dc-text)">${esc(m.username||'Unknown')}</span><span style="font-size:11px;color:var(--dc-muted)">${formatTime(m.created_at)}</span>${m.user_id===currentUserId?`<button onclick="deleteThreadMsg(${m.id},this.closest('[data-thread-msg-id]'))" style="margin-left:auto;background:none;border:none;cursor:pointer;color:#ef4444;font-size:12px;">🗑</button>`:''}</div><div style="font-size:14px;color:var(--dc-text);line-height:1.5;">${renderText(m.text||'')}</div>`;
  list.appendChild(el);if(!skipScroll)list.scrollTop=list.scrollHeight;
}
window.deleteThreadMsg=async(msgId,el)=>{
  if(!confirm('Delete this reply?'))return;
  await sb.from('messages').update({deleted_at:new Date().toISOString()}).eq('id',msgId);el?.remove();
};
...
async function sendThreadMsg(){
  ...
  const{error}=await sb.from('messages').insert({
    user_id:currentUserId,username:p.username,avatar_url:p.avatar_url||null,tag:p.tag||null,role:p.role||'user',
    text:filterProfanity(applyShortcodes(text)),thread_id:activeThreadId,
...
async function loadPins(){
  ...
  data.forEach(pin=>{
    ...
    item.prepend(del);item.onclick=e=>{if(e.target===del)return;jumpToMsg(pin.message_id);};
...
async function loadMembers(){
  ...
  if(!activeRoom.serverId){list.innerHTML=`<div style="padding:16px;font-size:13px;color:var(--dc-muted);">Open a server to see its members.</div>`;return;}
  ...
  const onlineUids=new Set(Object.values(presenceState).flat().map(u=>u.uid));
  const groups={Online:[],Offline:[]};
  profiles.forEach((p,i)=>{const uid=uids[i];const entry={p,uid,isOnline:onlineUids.has(uid),isOwner:uid===ownerId};if(entry.isOnline)groups.Online.push(entry);else groups.Offline.push(entry);});
  Object.entries(groups).forEach(([group,items])=>{
    if(!items.length)return;
    const sec=document.createElement('div');sec.className='member-role-section';sec.textContent=`${group} — ${items.length}`;list.appendChild(sec);
    items.forEach(({p,uid,isOnline,isOwner})=>{
      ...
      const name=document.createElement('span');name.textContent=p.username||'User';name.style.cssText='font-size:13px;font-weight:600;display:block;';
      const sub=document.createElement('div');sub.style.cssText='font-size:11px;color:var(--dc-muted);';
      const badges=[];if(isOwner)badges.push('👑 Owner');else if(p.role==='admin')badges.push('🛡 Admin');else if(p.role==='mod')badges.push('⚔️ Mod');if(p.tag)badges.push(p.tag);
      /* Game badge in member list */
      const presUser=Object.values(presenceState).flat().find(u=>u.uid===uid);
      if(presUser?.game)badges.push((GAME_EMOJIS[presUser.game]||'🎮')+' '+presUser.game);
      sub.textContent=badges.join(' · ')||'Member';
      nameWrap.appendChild(name);nameWrap.appendChild(sub);
      const dot=document.createElement('div');dot.className='mi-status'+(isOnline?' online':'');
...
document.getElementById('onlinePill')?.addEventListener('click',()=>{
  initOnlinePanel();const p=document.getElementById('online-panel');
  if(p.classList.contains('hidden')){p.classList.remove('hidden');renderOnlineList();}else p.classList.add('hidden');
});
function renderOnlineList(){
  const list=document.getElementById('online-list');if(!list)return;list.innerHTML='';
  ...
  users.forEach(u=>{
    ...
    if(u.avatar_url){const img=document.createElement('img');img.src=u.avatar_url;av.appendChild(img);}else av.textContent=getInitials(u.username||'?');
    const info=document.createElement('div');info.className='online-info';
    info.innerHTML=`<span class="online-name">${esc(u.username||'User')}</span>`;
    /* Game shown in online list */
    if(u.game){const gspan=document.createElement('div');gspan.style.cssText='font-size:11px;color:var(--dc-muted);';gspan.textContent=(GAME_EMOJIS[u.game]||'🎮')+' Playing '+u.game;info.appendChild(gspan);}
...
function renderTyping(){
  const el=document.getElementById('dc-typing');const users=Object.values(typingUsers);
  if(!users.length){el.innerHTML='';return;}
  const names=users.slice(0,3).map(u=>u.username).join(', ');
  el.innerHTML=`<div class="dc-typing-dots"><span></span><span></span><span></span></div><span>${esc(users.length===1?`${names} is typing`:users.length<=3?`${names} are typing`:`${users.length} people are typing`)}…</span>`;
}
...
══════════════════════════════════════════════════════ */
const CMDS=[
  {c:'/me',       mod:false,desc:'Send an action message'},
  {c:'/shrug',    mod:false,desc:'¯\\_(ツ)_/¯'},
  {c:'/lenny',    mod:false,desc:'( ͡° ͜ʖ ͡°)'},
  {c:'/tableflip',mod:false,desc:'(╯°□°）╯︵ ┻━┻'},
  {c:'/unflip',   mod:false,desc:'┬─┬ノ( º _ ºノ)'},
  {c:'/help',     mod:false,desc:'List commands'},
  {c:'/clear',    mod:true, desc:'Clear visible messages'},
  {c:'/slow',     mod:true, desc:'Slow mode (seconds)'},
  {c:'/warn',     mod:true, desc:'/warn <user> [reason]'},
  {c:'/mute',     mod:true, desc:'/mute <user> <mins>'},
  {c:'/unmute',   mod:true, desc:'/unmute <user>'},
  {c:'/ban',      mod:true, desc:'/ban <user> [reason]'},
  {c:'/unban',    mod:true, desc:'/unban <user>'},
  {c:'/promote',  mod:true, desc:'/promote <user>'},
  {c:'/demote',   mod:true, desc:'/demote <user>'},
  {c:'/announce', mod:true, desc:'/announce <text>'},
];
function renderSlashSugg(query){
  ...
  slashSuggIdx=Math.min(slashSuggIdx,matches.length-1);popup.classList.remove('hidden');popup.innerHTML='';
...
msgInput.addEventListener('input',()=>{
  ...
  hideSlashPopup();
  const pos=msgInput.selectionStart||0;const before=val.slice(0,pos);
...
msgInput.onkeydown=e=>{
  ...
  if(slashPopup&&!slashPopup.classList.contains('hidden')){
    ...
    if(e.key==='Tab'||e.key==='Enter'){const a=slashPopup.querySelector('.slash-item.active');if(a){e.preventDefault();applySlashSugg(a.querySelector('.slash-cmd').textContent);return;}}
    if(e.key==='Escape'){hideSlashPopup();return;}
  }
  const mentPopup=document.getElementById('dc-mention-popup');
  if(e.key==='ArrowUp'&&mentPopup.children.length){e.preventDefault();mentionSelIdx=Math.max(0,mentionSelIdx-1);handleMentionAC();return;}
  if(e.key==='ArrowDown'&&mentPopup.children.length){e.preventDefault();mentionSelIdx=Math.min(7,mentionSelIdx+1);handleMentionAC();return;}
  if(e.key==='Tab'&&mentPopup.children.length){e.preventDefault();const a=document.querySelector('.dc-mention-item.active');if(a)insertMention(a.dataset.user);return;}
  if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();sendMessage();}
  if(e.key==='Escape'){mentPopup.innerHTML='';hideSlashPopup();}
};
...
async function sendMessage(){
  ...
  /* FIX: always re-fetch profile fresh to get current mute/ban status */
  ...
  if(await isMutedFresh(session.user.id)){showToast(`🔇 You are muted for another ${muteExpiryText(p)}.`);return;}

  /* Ephemeral bot intercepts */
  if(/^(!ping|@ping)\b/i.test(text)){
    msgInput.value='';msgInput.style.height='auto';hideSlashPopup();
    showEphemeral('🏓 Pinging…','🤖','AutoBot');
    try{
    ...
    }catch{showEphemeral('🏓 Pong! (could not measure latency)','🏓','PingBot — Only visible to you');}
    return;
  }
  if(/@automod/i.test(text)){
    ...
    }catch{showEphemeral('AutoMod is offline right now. Try again later.','🤖','AutoMod — Only visible to you');}
    return;
  }
...
async function runCommand(text,p){
  ...
  switch(cmd){
    ...
    case '/help':showToast(CMDS.filter(c=>!c.mod||isAdminOrMod(p)).map(c=>c.c).join(' · '),5000);break;
    case '/clear':if(!isAdminOrMod(p)){showToast('❌ Mods only');return;}document.getElementById('dc-messages').innerHTML='';msgElMap.clear();break;
    case '/slow':if(!isAdminOrMod(p)){showToast('❌ Mods only');return;}slowModeSeconds=parseInt(args)||0;showToast(slowModeSeconds?`🐌 Slow: ${slowModeSeconds}s`:'✅ Slow mode off');break;
    case '/warn':case '/mute':case '/unmute':case '/ban':case '/unban':case '/promote':case '/demote':case '/announce':
      if(!isAdminOrMod(p)){showToast('❌ Mods only');return;}await runModCmd(cmd,args,p,payload);break;
...
async function runModCmd(cmd,args,p,payload){
  ...
  switch(cmd){
    case '/warn':{const reason=args.split(' ').slice(1).join(' ')||'No reason';const nc=(tgt.warn_count||0)+1;await sb.from('profiles').update({warn_count:nc}).eq('id',tgt.id);let extra='';if(nc>=5){await sb.from('profiles').update({banned:true}).eq('id',tgt.id);extra=' (auto-banned)';}else if(nc>=3){const mu=new Date(Date.now()+60*60000).toISOString();await sb.from('profiles').update({muted_until:mu}).eq('id',tgt.id);extra=' (auto-muted 1h)';}await logAutomod(tgt.id,tgt.username,'warn',reason);payload.text=`⚠️ **Warning ${nc}/5** to @${tgt.username}: ${reason}${extra}`;await insertMsg(payload);break;}
    case '/mute':{const mins=parseInt(args.split(' ')[1])||5;const reason=args.split(' ').slice(2).join(' ')||'No reason';const mu=new Date(Date.now()+mins*60000).toISOString();await sb.from('profiles').update({muted_until:mu}).eq('id',tgt.id);delete profileCache[tgt.id];await logAutomod(tgt.id,tgt.username,'mute',`${mins}m — ${reason}`,mu);payload.text=`🔇 @${tgt.username} muted for ${mins}m: ${reason}`;await insertMsg(payload);break;}
    case '/unmute':await sb.from('profiles').update({muted_until:null}).eq('id',tgt.id);delete profileCache[tgt.id];await logAutomod(tgt.id,tgt.username,'unmute',`by ${p.username}`);showToast('✅ Unmuted');break;
    case '/ban':{const reason=args.split(' ').slice(1).join(' ')||'No reason';if(tgt.role==='admin'){showToast('❌ Cannot ban admin');return;}await sb.from('profiles').update({banned:true}).eq('id',tgt.id);await logAutomod(tgt.id,tgt.username,'ban',reason);payload.text=`🚫 @${tgt.username} banned: ${reason}`;await insertMsg(payload);break;}
    ...
    case '/promote':if(p.role!=='admin'){showToast('❌ Admins only');return;}await sb.from('profiles').update({role:'mod'}).eq('id',tgt.id);showToast('✅ Promoted to mod');break;
    case '/demote':if(p.role!=='admin'){showToast('❌ Admins only');return;}await sb.from('profiles').update({role:'user'}).eq('id',tgt.id);showToast('✅ Demoted');break;
...
async function showProfilePopup(userId,anchorEl){
  ...
  /* Game status in profile popup */
  let gline=popup.querySelector('.pp-game-line');
  if(!gline){gline=document.createElement('div');gline.className='pp-game-line';gline.style.cssText='font-size:12px;color:var(--dc-muted);margin-bottom:8px;min-height:0;';document.getElementById('pp-email').after(gline);}
  const presUser=Object.values(presenceState).flat().find(u=>u.uid===userId);
  gline.textContent=presUser?.game?`${GAME_EMOJIS[presUser.game]||'🎮'} Playing ${presUser.game}`:'';
  ...
  /* FIX: clamp popup within viewport */
...
async function startDMWith(emailOrUsername){
  ...
  if(ex){dmId=ex.id;}
    ...
    try{const{data:nd,error}=await sb.from('direct_messages').insert({user_a:currentUserId,user_b:profile.id}).select().single();if(error){document.getElementById('dm-err').textContent=error.message;return;}dmId=nd.id;}
    catch(e){document.getElementById('dm-err').textContent=String(e?.message||e);return;}
  }
...
function openServerModal(server){
  ...
  let iconWrap=document.getElementById('sm-icon-wrap');
  if(!iconWrap){
    const modal=document.querySelector('#serverModal .dc-modal');
    iconWrap=document.createElement('div');iconWrap.id='sm-icon-wrap';iconWrap.style.cssText='display:flex;align-items:center;gap:14px;margin-bottom:16px;';
    const preview=document.createElement('div');preview.id='sm-icon-preview';preview.style.cssText='width:72px;height:72px;border-radius:16px;background:var(--dc-input-bg);display:flex;align-items:center;justify-content:center;font-size:32px;cursor:pointer;overflow:hidden;position:relative;border:2px solid var(--dc-sep);';
    const ov=document.createElement('div');ov.className='icon-overlay';ov.style.cssText='position:absolute;inset:0;background:rgba(0,0,0,.4);display:flex;align-items:center;justify-content:center;font-size:20px;opacity:0;transition:opacity .15s;';ov.textContent='📷';
    preview.appendChild(ov);preview.addEventListener('mouseenter',()=>ov.style.opacity='1');preview.addEventListener('mouseleave',()=>ov.style.opacity='0');
    const meta=document.createElement('div');
    meta.innerHTML=`<div style="font-weight:700;font-size:14px;margin-bottom:4px;">Server Icon</div><div style="font-size:12px;color:var(--dc-muted);margin-bottom:8px;">Max 1 MB · JPG, PNG, GIF, WebP</div><input type="file" id="sm-icon-file" accept="image/jpeg,image/png,image/gif,image/webp" style="display:none;"><label for="sm-icon-file" style="cursor:pointer;font-size:12px;color:var(--a);font-weight:600;">Upload Image</label>`;
    iconWrap.appendChild(preview);iconWrap.appendChild(meta);
    modal.insertBefore(iconWrap,modal.querySelector('h3').nextSibling);
    document.getElementById('sm-icon-file').addEventListener('change',e=>{
      const f=e.target.files[0];if(!f)return;
      if(f.size>1*1024*1024){showToast('❌ Image must be under 1 MB');e.target.value='';return;}
      pendingServerIconFile=f;
      const reader=new FileReader();reader.onload=ev=>{const p=document.getElementById('sm-icon-preview');p.innerHTML='';const img=document.createElement('img');img.src=ev.target.result;img.style.cssText='width:100%;height:100%;object-fit:cover;';p.appendChild(img);const ov2=document.createElement('div');ov2.className='icon-overlay';ov2.style.cssText='position:absolute;inset:0;background:rgba(0,0,0,.4);display:flex;align-items:center;justify-content:center;font-size:20px;opacity:0;transition:opacity .15s;';ov2.textContent='📷';p.appendChild(ov2);p.addEventListener('mouseenter',()=>ov2.style.opacity='1');p.addEventListener('mouseleave',()=>ov2.style.opacity='0');};reader.readAsDataURL(f);
    });
    preview.addEventListener('click',()=>document.getElementById('sm-icon-file').click());
  }
  const prev=document.getElementById('sm-icon-preview');prev.innerHTML='';
  const ov2=document.createElement('div');ov2.style.cssText='position:absolute;inset:0;background:rgba(0,0,0,.4);display:flex;align-items:center;justify-content:center;font-size:20px;opacity:0;transition:opacity .15s;';ov2.textContent='📷';
  if(server?.icon&&(server.icon.startsWith('http')||server.icon.startsWith('/'))){const img=document.createElement('img');img.src=server.icon;img.style.cssText='width:100%;height:100%;object-fit:cover;';prev.appendChild(img);}
  else prev.textContent=server?.icon||'🌐';
  prev.appendChild(ov2);prev.addEventListener('mouseenter',()=>ov2.style.opacity='1');prev.addEventListener('mouseleave',()=>ov2.style.opacity='0');
  const btn=document.getElementById('sm-submit');btn.onclick=isEdit?()=>saveServer(server):()=>createServer();
  openModal('serverModal');
}
async function uploadServerIcon(file){
  const ext=file.name.split('.').pop().toLowerCase();const path=`${currentUserId}/${Date.now()}.${ext}`;
...
async function createServer(){
  ...
  let iconUrl=null;if(pendingServerIconFile){iconUrl=await uploadServerIcon(pendingServerIconFile);if(!iconUrl){btn.disabled=false;btn.textContent='Create';return;}}
  ...
  joinedServerIds.add(server.id);pendingServerIconFile=null;closeModal('serverModal');btn.disabled=false;btn.textContent='Create';
  await buildRail();await enterServer(server);
}
async function saveServer(server){
  const name=document.getElementById('sm-name').value.trim();if(!name){document.getElementById('sm-err').textContent='Name required.';return;}
  const btn=document.getElementById('sm-submit');btn.disabled=true;btn.textContent='Saving…';
  let iconUrl=server.icon||null;if(pendingServerIconFile){iconUrl=await uploadServerIcon(pendingServerIconFile);if(!iconUrl){btn.disabled=false;btn.textContent='Save';return;}}
...
function openInvitePanel(server){
  ...
  closeAllPanels();panel.classList.remove('hidden');loadInvites(server.id);
}
...
document.getElementById('jm-join')?.addEventListener('click',async()=>{
  ...
  const serverId=document.getElementById('jm-join')?.dataset.serverId;if(!serverId)return;
...
function trackUnread(msg){
  ...
  if(msg.server_id){
    const sKey='server:'+msg.server_id;unreadCounts[sKey]=(unreadCounts[sKey]||0)+1;
    const railBtn=document.querySelector(`.rail-server-icon[data-server-id="${msg.server_id}"]`);
    if(railBtn){railBtn.classList.add('unread');let pip=railBtn.querySelector('.rail-unread');if(!pip){pip=document.createElement('span');pip.className='rail-unread';railBtn.appendChild(pip);}pip.textContent=unreadCounts[sKey]>9?'9+':String(unreadCounts[sKey]);}
  }
...
async function markRoomRead(room){
  ...
  if(room.serverId){unreadCounts['server:'+room.serverId]=0;const railBtn=document.querySelector(`.rail-server-icon[data-server-id="${room.serverId}"]`);railBtn?.classList.remove('unread');railBtn?.querySelector('.rail-unread')?.remove();}
...
/* ══════════════════════════════════════════════════════
   GAME OVERLAY CSS (injected once)
══════════════════════════════════════════════════════ */
(()=>{
  const style=document.createElement('style');
  style.textContent=`
@keyframes goSlideIn{from{opacity:0;transform:translateX(120%)}to{opacity:1;transform:translateX(0)}}
.slash-popup{position:absolute;bottom:calc(100% + 6px);left:0;right:0;background:var(--dc-side-bg);border:1px solid var(--dc-sep);border-radius:10px;overflow:hidden;box-shadow:0 -4px 24px rgba(0,0,0,.2);z-index:200;max-height:260px;overflow-y:auto;}
.slash-popup.hidden{display:none;}
.slash-item{display:flex;align-items:center;gap:10px;padding:9px 14px;cursor:pointer;transition:background .1s;}
.slash-item:hover,.slash-item.active{background:var(--dc-ch-hover);}
.slash-cmd{font-family:ui-monospace,monospace;font-size:13px;font-weight:700;color:var(--a);min-width:120px;}
.slash-desc{font-size:12px;color:var(--dc-muted);}
.dc-ephemeral{display:flex;align-items:flex-start;gap:12px;padding:8px 16px 8px 20px;background:rgba(99,102,241,.07);border-left:3px solid #6366f1;margin:2px 0;}
.dc-ephemeral-icon{font-size:20px;flex-shrink:0;margin-top:2px;}
.dc-ephemeral-body{flex:1;min-width:0;}
.dc-ephemeral-label{font-size:11px;font-weight:700;color:#6366f1;text-transform:uppercase;letter-spacing:.04em;margin-bottom:3px;}
.dc-ephemeral-text{font-size:14px;color:var(--dc-text);line-height:1.5;word-break:break-word;}
.dc-ephemeral-dismiss{background:none;border:none;cursor:pointer;color:var(--dc-muted);font-size:15px;padding:0 4px;opacity:.5;flex-shrink:0;margin-top:2px;}
.dc-ephemeral-dismiss:hover{opacity:1;color:#ef4444;}
.online-panel{position:absolute;right:0;top:48px;bottom:0;width:240px;z-index:90;background:var(--dc-side-bg);border-left:1px solid var(--dc-sep);display:flex;flex-direction:column;}
.online-panel.hidden{display:none;}
.online-header{height:44px;display:flex;align-items:center;justify-content:space-between;padding:0 14px;border-bottom:1px solid var(--dc-sep);font-size:14px;font-weight:700;color:var(--dc-text);}
.online-header button{background:none;border:none;cursor:pointer;color:var(--dc-muted);font-size:16px;padding:4px;border-radius:6px;}
.online-list{flex:1;overflow-y:auto;padding:8px;}
.online-item{display:flex;align-items:center;gap:9px;padding:5px 8px;border-radius:6px;}
.online-av{width:28px;height:28px;border-radius:50%;background:var(--a);color:#fff;font-size:12px;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0;overflow:hidden;}
.online-av img{width:100%;height:100%;object-fit:cover;}
.online-name{font-size:13px;font-weight:600;color:var(--dc-text);}
.invite-panel{position:absolute;right:0;top:48px;bottom:0;width:320px;z-index:90;background:var(--dc-main-bg);border-left:1px solid var(--dc-sep);display:flex;flex-direction:column;}
.invite-panel.hidden{display:none;}
.invite-header{height:44px;display:flex;align-items:center;justify-content:space-between;padding:0 14px;border-bottom:1px solid var(--dc-sep);font-size:14px;font-weight:700;color:var(--dc-text);}
.invite-header button{background:none;border:none;cursor:pointer;color:var(--dc-muted);font-size:16px;padding:4px;border-radius:6px;}
.invite-body{flex:1;overflow-y:auto;padding:14px;}
.invite-item{padding:10px 12px;border-radius:8px;margin-bottom:10px;background:var(--dc-input-bg);border:1px solid var(--dc-sep);}
.invite-code{font-family:ui-monospace,monospace;font-size:14px;font-weight:700;color:var(--a);}
.server-ctx-menu{position:fixed;z-index:999;background:var(--dc-header-bg);border:1px solid var(--dc-sep);border-radius:10px;padding:4px;min-width:180px;box-shadow:0 8px 24px rgba(0,0,0,.2);}
.ch-unread-badge{min-width:18px;height:18px;border-radius:999px;background:#ef4444;color:#fff;font-size:10px;font-weight:700;display:flex;align-items:center;justify-content:center;padding:0 5px;flex-shrink:0;margin-left:auto;}
`;
...
(async()=>{
  try{
    ...
    sb.auth.onAuthStateChange(async(_,session)=>{
      currentUserId=session?.user?.id||null;
      if(currentUserId){delete profileCache[currentUserId];currentProfile=await getProfile(currentUserId);}
      else currentProfile=null;
      updateUserChip();await buildRail();
      if(currentUserId)startPresence();
    });
