/* ════════════════════════════════════════════════════════
   360 Chat — v3.3
   Fixes: threads (count/display), sidebar menu, members,
   server images, joined-only rail, slash suggestions,
   unread badges, ephemeral bot messages, invite management
════════════════════════════════════════════════════════ */
...
function formatDate(ts){const d=new Date(ts),t=new Date(),y=new Date();y.setDate(t.getDate()-1);if(d.toDateString()===t.toDateString())return'Today';if(d.toDateString()===y.toDateString())return'Yesterday';return d.toLocaleDateString([],{month:'long',day:'numeric',year:'numeric'});}
...
function showEphemeral(text, icon='🤖', label='Only you can see this'){
  ...
  el.innerHTML = `
    <div class="dc-ephemeral-icon">${icon}</div>
...
async function buildRail(){
  ...
  gen.title='General';gen.dataset.serverId='';
  gen.innerHTML='🌐';
  ...
  // Fetch all public servers + joined locked servers
  // Admins/mods see ALL servers; others see public + joined
  let serversQuery = sb.from('servers').select('*').order('name');
  const{data:allServers}=await serversQuery;
  const servers = (allServers||[]).filter(s=>{
    if(isAdminOrMod(currentProfile)) return true; // admins/mods see all
    if(joinedServerIds.has(s.id)) return true;    // joined servers always shown
    return !s.passcode;                            // public servers shown to everyone
  });
  ...
  servers.forEach(s=>{
    ...
    // Server icon: image URL or emoji/letter
...
async function buildSidebar(server){
  ...
  if(!server){
    ...
    const item=makeChanItem({id:'public',name:'general'},activeRoom.type==='public',()=>switchRoom({type:'public',id:'public',name:'general',icon:'#',serverName:'360 Chat',serverId:null}));
  ...
  if(server.owner_id===currentUserId||isAdminOrMod(currentProfile)){
  ...
  if(currentUserId&&!joinedServerIds.has(server.id)){
  ...
  if(!dms?.length){body.innerHTML=`<div style="padding:20px;font-size:13px;color:var(--dc-muted);text-align:center;">No DMs yet.</div>`;}
    ...
    dms.forEach((dm,i)=>{
      ...
      if(unreadCounts[dmKey]>0){
        const badge=document.createElement('span');badge.className='ch-unread-badge';
        badge.textContent=unreadCounts[dmKey]>99?'99+':String(unreadCounts[dmKey]);
        item.appendChild(badge);
      }
...
async function browseSidebar(body){
  ...
  (allServers||[]).forEach(s=>{
    ...
    item.innerHTML=`${iconHtml}<span>${esc(s.name)}</span>${isJoined?`<span style="margin-left:auto;font-size:11px;color:var(--a);">✓</span>`:isLocked?`<span style="margin-left:auto;font-size:11px;opacity:.5;">🔒</span>`:''}`;
    item.addEventListener('click',()=>handleServerClick(s));body.appendChild(item);
  });
}

/* ══════════════════════════════════════════════════════
   SERVER CONTEXT MENU (the ⌄ button)
══════════════════════════════════════════════════════ */
document.getElementById('sb-server-menu')?.addEventListener('click',async(e)=>{
  ...
  menu.id='server-ctx-menu';menu.className='server-ctx-menu';

  const items=[
    {label:'📋 Copy Server ID',fn:()=>{navigator.clipboard.writeText(server.id);showToast('Copied!');}}
  ];
  if(canManage){
  ...
  if(!isOwner){
    if(joinedServerIds.has(server.id))
      items.push({label:'🚪 Leave Server',danger:true,fn:()=>leaveServer(server)});
  }
  ...
  items.forEach(item=>{
    ...
    d.textContent=item.label;d.onclick=()=>{menu.remove();item.fn();};
    menu.appendChild(d);
  });
...
async function handleServerClick(server){
  ...
  if(isJoined||isAdminOrMod(currentProfile)){
    await enterServer(server);
  }else if(server.passcode){
    await buildSidebar(server);showPasscodeGate(server);
  }else{
...
function switchRoom(room){
  ...
  if(room.type==='channel'){
    // CRITICAL: filter thread_id IS NULL so replies don't appear in main chat
    chan.on('postgres_changes',{event:'INSERT',schema:'public',table:'messages',filter:`channel_id=eq.${room.id}`},p=>{
        if(p.new.thread_id!=null) return; // skip thread replies in main chat
        onIncoming(p.new);
      })
      .on('postgres_changes',{event:'UPDATE',schema:'public',table:'messages',filter:`channel_id=eq.${room.id}`},p=>{
        // Update thread pill count if this is a thread root
        if(p.new.is_thread_root){
          const el=msgElMap.get(String(p.new.id));
          if(el)updateThreadPill(el,p.new);
        }
...
function buildMsgEl(msg){
  ...
  // Thread replies NEVER appear in main chat
  ...
  // Thread pill — shows reply count, live-updated via UPDATE subscription
  const pill=document.createElement('div');pill.className='dc-thread-pill';
  if((msg.thread_reply_count||0)>0||msg.is_thread_root){
    pill.textContent=msg.thread_reply_count>0?`🧵 ${msg.thread_reply_count} repl${msg.thread_reply_count===1?'y':'ies'}`:'🧵 Start Thread';
    pill.style.display='';
  }else{
...
function updateThreadPill(el, msg){
  ...
  if(count>0||msg.is_thread_root){
    pill.textContent=count>0?`🧵 ${count} repl${count===1?'y':'ies'}`:'🧵 Start Thread';
    pill.style.display='';
  }
}
...
/* ══════════════════════════════════════════════════════
   THREADS — fixed: only saves to thread, count via DB trigger
══════════════════════════════════════════════════════ */
function openThread(msg){
  // Re-fetch latest msg state so count is accurate
  ...
  // Subscribe to new thread replies live
  if(window._threadRtChan)sb.removeChannel(window._threadRtChan);
  window._threadRtChan=sb.channel('thread-'+msg.id)
    .on('postgres_changes',{event:'INSERT',schema:'public',table:'messages',filter:`thread_id=eq.${msg.id}`},p=>{
      appendThreadMsg(p.new);
    })
    .subscribe();
}
...
async function loadThreadMsgs(rootId){
  ...
  data.forEach(m=>appendThreadMsg(m,true));
...
async function sendThreadMsg(){
  ...
  // Insert with thread_id — DB trigger auto-increments thread_reply_count on root
  ...
  // Thread pill update comes via UPDATE realtime subscription on the root message
}
...
async function loadPins(){
  ...
  data.forEach(pin=>{
    ...
    item.prepend(del);
    item.onclick=e=>{if(e.target===del)return;jumpToMsg(pin.message_id);};
...
/* ══════════════════════════════════════════════════════
   MEMBERS — shows real online status via presence
══════════════════════════════════════════════════════ */
async function loadMembers(){
  ...
  // Fetch the current server to know owner
  ...
  profiles.forEach((p,i)=>{
    ...
    const entry={p,uid,isOnline,isOwner:uid===ownerId};
    if(isOnline)groups.Online.push(entry);else groups.Offline.push(entry);
  });
  ...
  Object.entries(groups).forEach(([group,items])=>{
    ...
    items.forEach(({p,uid,isOnline,isOwner})=>{
      ...
      const name=document.createElement('span');name.textContent=p.username||'User';name.style.fontSize='13px';name.style.fontWeight='600';
      const sub=document.createElement('div');sub.style.cssText='font-size:11px;color:var(--dc-muted);';
      const badges=[];
      if(isOwner)badges.push('👑 Owner');
      else if(p.role==='admin')badges.push('🛡 Admin');
      else if(p.role==='mod')badges.push('⚔️ Mod');
      if(p.tag)badges.push(p.tag);
...
function renderOnlineList(){
  ...
  users.forEach(u=>{
    ...
    item.innerHTML=``;item.appendChild(av);
...
function applySlashSugg(cmdStr){
  const inp=document.getElementById('msgInput');inp.value=cmdStr+' ';inp.focus();hideSlashPopup();
}
...
msgInput.addEventListener('input',()=>{
  ...
  hideSlashPopup();
  const before=val.slice(0,pos);if(/@\w*$/.test(before))handleMentionAC();
...
/* ══════════════════════════════════════════════════════
   SEND — intercepts bot keywords ephemerally
══════════════════════════════════════════════════════ */
...
async function sendMessage(){
  ...
  // !ping / @ping → ephemeral latency reply, nothing saved
  if(/^(!ping|@ping)\b/i.test(text)){
    msgInput.value='';msgInput.style.height='auto';hideSlashPopup();
    showEphemeral('🏓 Pinging…','🤖','AutoBot');
    try{
      const sentAt=Date.now();
      const res=await fetch(`${SB_URL}/functions/v1/pingpong`,{
        method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({username:p.username,sent_at:sentAt})
      });
      const d=await res.json();
      // Remove the "pinging" ephemeral and show the real one
  ...
  // @automod → ephemeral AI reply, nothing saved
  if(/@automod/i.test(text)){
    msgInput.value='';msgInput.style.height='auto';hideSlashPopup();
    showEphemeral('⏳ AutoMod is thinking…','🤖','AutoMod — Only visible to you');
    try{
      const res=await fetch(`${SB_URL}/functions/v1/automod`,{
        method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({mode:'chat',text,username:p.username,user_id:currentUserId,channel_id:activeRoom.type==='channel'?activeRoom.id:null,server_id:activeRoom.serverId||null})
      });
      const d=await res.json();
      document.querySelector('.dc-ephemeral:last-child')?.remove();
...
async function runCommand(text,p){
  ...
  switch(cmd){
    ...
    case '/clear':
      if(!isAdminOrMod(p)){showToast('❌ Mods only');return;}
      document.getElementById('dc-messages').innerHTML='';msgElMap.clear();break;
    case '/slow':
      if(!isAdminOrMod(p)){showToast('❌ Mods only');return;}
      slowModeSeconds=parseInt(args)||0;showToast(slowModeSeconds?`🐌 Slow: ${slowModeSeconds}s`:'✅ Slow mode off');break;
    case '/warn':case '/mute':case '/unmute':case '/ban':case '/unban':case '/promote':case '/demote':case '/announce':
      if(!isAdminOrMod(p)){showToast('❌ Mods only');return;}
      await runModCmd(cmd,args,p,payload);break;
...
async function runModCmd(cmd,args,p,payload){
  ...
  if(cmd!=='/announce'&&cmd!=='/slow'&&!tgt){showToast('❌ User not found');return;}
  switch(cmd){
    case '/warn':{
      const reason=args.split(' ').slice(1).join(' ')||'No reason';
      const newCount=(tgt.warn_count||0)+1;
      await sb.from('profiles').update({warn_count:newCount}).eq('id',tgt.id);
      let extra='';
      if(newCount>=5){await sb.from('profiles').update({banned:true}).eq('id',tgt.id);extra=' (auto-banned)';}
      else if(newCount>=3){const mu=new Date(Date.now()+60*60000).toISOString();await sb.from('profiles').update({muted_until:mu}).eq('id',tgt.id);extra=' (auto-muted 1h)';}
      await logAutomod(tgt.id,tgt.username,'warn',reason);
      payload.text=`⚠️ **Warning ${newCount}/5** to @${tgt.username}: ${reason}${extra}`;await insertMsg(payload);break;
    }
    case '/mute':{
      const mins=parseInt(args.split(' ')[1])||5;const reason=args.split(' ').slice(2).join(' ')||'No reason';
      const mu=new Date(Date.now()+mins*60000).toISOString();
      await sb.from('profiles').update({muted_until:mu}).eq('id',tgt.id);
      await logAutomod(tgt.id,tgt.username,'mute',`${mins}m — ${reason}`,mu);
      payload.text=`🔇 @${tgt.username} muted for ${mins}m: ${reason}`;await insertMsg(payload);break;
    }
    case '/unmute':
      await sb.from('profiles').update({muted_until:null}).eq('id',tgt.id);
      await logAutomod(tgt.id,tgt.username,'unmute',`by ${p.username}`);showToast('✅ Unmuted');break;
    case '/ban':{
      const reason=args.split(' ').slice(1).join(' ')||'No reason';
      if(tgt.role==='admin'){showToast('❌ Cannot ban admin');return;}
      await sb.from('profiles').update({banned:true}).eq('id',tgt.id);
      await logAutomod(tgt.id,tgt.username,'ban',reason);
      payload.text=`🚫 @${tgt.username} banned: ${reason}`;await insertMsg(payload);break;
    }
    case '/unban':
      await sb.from('profiles').update({banned:false,warn_count:0}).eq('id',tgt.id);
      await logAutomod(tgt.id,tgt.username,'unban','');showToast('✅ Unbanned');break;
    case '/promote':
      if(p.role!=='admin'){showToast('❌ Admins only');return;}
      await sb.from('profiles').update({role:'mod'}).eq('id',tgt.id);showToast('✅ Promoted to mod');break;
    case '/demote':
      if(p.role!=='admin'){showToast('❌ Admins only');return;}
      await sb.from('profiles').update({role:'user'}).eq('id',tgt.id);showToast('✅ Demoted');break;
    case '/announce':
      payload.text=`📢 **${args}**`;await insertMsg(payload);break;
  }
}
...
/* ══════════════════════════════════════════════════════
   SERVER MODAL — with image upload for icon
══════════════════════════════════════════════════════ */
function openServerModal(server){
  ...
  if(!iconWrap){
    const field=document.querySelector('.dc-field:has(#sm-icon)') || document.getElementById('sm-icon')?.closest('.dc-field');
    // Inject icon upload widget before name field in modal
    const modal=document.querySelector('#serverModal .dc-modal');
    iconWrap=document.createElement('div');
    iconWrap.id='sm-icon-wrap';
    iconWrap.style.cssText='display:flex;align-items:center;gap:14px;margin-bottom:16px;';
    const preview=document.createElement('div');preview.className='server-icon-preview';preview.id='sm-icon-preview';
    const overlay=document.createElement('div');overlay.className='icon-overlay';overlay.textContent='📷';
    ...
    meta.innerHTML=`<div style="font-weight:700;font-size:14px;margin-bottom:4px;">Server Icon</div>
      ...
      <label for="sm-icon-file" style="cursor:pointer;font-size:12px;color:var(--a);font-weight:600;">Upload Image</label>`;
    iconWrap.appendChild(preview);iconWrap.appendChild(meta);
    modal.insertBefore(iconWrap,modal.firstChild.nextSibling||modal.firstChild);
...
async function uploadServerIcon(file){
  ...
  const{data:u}=sb.storage.from('server-icons').getPublicUrl(path);
...
async function createServer(){
  ...
  closeModal('serverModal');btn.disabled=false;btn.textContent='Create';
...
function startPresence(){
  ...
  chan.on('presence',{event:'sync'},()=>{
    ...
    // Refresh members panel online dots if open
...
function trackUnread(msg){
  ...
  if(msg.thread_id!=null)return; // thread replies don't count as channel unread
...
async function markRoomRead(room){
  ...
  if(room.serverId){
    ...
    railBtn?.classList.remove('unread');railBtn?.querySelector('.rail-unread')?.remove();
  }
